...
<?php
// ── Access Guard ─────────────────────────────────────────────────────────────
if (!in_array($steno_access, ['Hindi', 'All', 'Both'])) {
...
try {
    ...
    $cats = $pdo->query(
        "SELECT c.*, COUNT(t.id) as test_count
         ...
         LEFT JOIN steno_tests t ON t.category_id = c.id AND t.language = 'Hindi' AND t.status = 1
         WHERE c.status = 1
         GROUP BY c.id
         ORDER BY c.name ASC"
    )->fetchAll();
                                    ...
                                    <?php if (empty($tests)): ?>
                                        <tr>
                                            <td colspan="5" class="text-center py-5 text-muted">
                                                ...
                                                <p>No Hindi steno tests found matching your criteria.</p>
                                            </td>
                                        </tr>
                                    <?php else: ?>
