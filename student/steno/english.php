...
<?php
// ── Access Guard ─────────────────────────────────────────────────────────────
if (!in_array($steno_access, ['English', 'All', 'Both'])) {
...
try {
    ...
    $cats = $pdo->query(
        "SELECT c.*, COUNT(t.id) as test_count
         FROM steno_exam_categories c
         LEFT JOIN steno_tests t ON t.category_id = c.id AND t.language = 'English' AND t.status = 1
         WHERE c.status = 1
         GROUP BY c.id
         ORDER BY c.name ASC"
    )->fetchAll();
    ...
    // Total count
    $total_stmt = $pdo->prepare("SELECT COUNT(t.id) FROM steno_tests t WHERE $where_sql");
                    ...
                    <div class="card-header bg-white p-3">
                        <form method="GET" class="row g-2 align-items-center">
                            <div class="col-md-4 d-flex align-items-center">
                                <h3 class="card-title" style="font-weight:700; color:#343a40; margin:0;">English Steno Tests (<?= $total_results ?>)</h3>
                            ...
                            <div class="col-md-2 text-end">
                                <?php if ($cat_filter || $level_filter || $search_filter): ?>
                                    ...
                                    <?php if (empty($tests)): ?>
                                        <tr>
                                            <td colspan="5" class="text-center py-5 text-muted">
                                                <i class="fas fa-pen-nib fa-3x mb-3 d-block" style="opacity: 0.2;"></i>
                                                <p>No English steno tests found matching your criteria.</p>
                                            </td>
                                        </tr>
                                    <?php else: ?>
